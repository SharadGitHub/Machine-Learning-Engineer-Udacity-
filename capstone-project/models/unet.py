...
class Encoder_block(nn.Module):
  ...
  def __init__(self, input_chn, output_chn, num_layers=2, kernel=3, padding=1, last_block=False):
    ...
    self.op = nn.Sequential(nn.Conv2d(layers_chn[0], layers_chn[1], kernel_size=kernel, padding=padding),
        ...
        nn.BatchNorm2d(layers_chn[3], affine=True),
        nn.ReLU(inplace=True))
...
class Decoder_block(nn.Module):
  ...
  def forward(self, x, feat_encoder):
    x = self.up(x)
    assert x.shape[1] == feat_encoder.shape[1], 'Channels should be same to concatenate'
...
class Unet(nn.Module):
  ...
  def __init__(self, num_classes):
    ...
    for i in range(5):
      if i == 4:
        # this makes sure that max pooling is not applied to the last block of encoder
        self.blocks.update(
            {'down_block' + str(i + 1): Encoder_block(down_chn[i], down_chn[i + 1], last_block=True)})
      else:
        # max pooling should be applied to rest of the blocks of encoder
        self.blocks.update(
  ...
  def forward(self, x):
    ...
    for i in range(5):
      ...
      encoder_results.append(feat)

    index = 3
    for i in range(4):
      feat = self.blocks['up_block' +
                      str(i + 1)](feat, encoder_results[index - i])
